...
public class StorageOperations {
    ...
    public void downloadBlobFromBucket(String bucketName, String blobName) {
        ...
        Path downloadTo = Paths.get("").toAbsolutePath().resolve(blobName);
        ...
        try  (PrintStream writeTo = new PrintStream(Files.newOutputStream(downloadTo))) {
            if (blob.getSize() < 1_000_000) {
                ...
                System.out.println("Blob " + blobName + " downloaded in bucket " + bucketName);
            } else {
    ...
    public void deleteBlob() throws IOException {
        Scanner scan = new Scanner(System.in);
    ...
    public void manageBucketPermissions() throws IOException{
        Scanner scan = new Scanner(System.in);
        System.out.println("Enter the name of the Bucket? ");
        String bucketName = scan.nextLine();
        Bucket bucket = storage.get(bucketName);
        ...
        bucket.createAcl(acl);
    }

    public void manageBlobPermissions() throws IOException{
        Scanner scan = new Scanner(System.in);
        System.out.println("Enter the name of the Bucket? ");
        String bucketName = scan.nextLine();
        Bucket bucket = storage.get(bucketName);
        ...
        // Get the ACL for the blob
        Acl.Entity aclEntity = Acl.User.ofAllAuthenticatedUsers();
        System.out.println("1. Reader");
        Acl.Role role = Acl.Role.READER;
        Acl acl = Acl.newBuilder(aclEntity, role).build();
        // Set the ACL for the blob
        blob.createAcl(acl);
