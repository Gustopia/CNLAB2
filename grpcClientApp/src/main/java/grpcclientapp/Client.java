...
public class Client {
    ...
    static void findPrimesCall()  throws InterruptedException  {
        int start = Integer.parseInt(read("First Number", new Scanner(System.in)));
        int end= Integer.parseInt(read("Last Number?", new Scanner(System.in)));

        FindPrimesStream fps = new FindPrimesStream();
        noBlockStub.findPrimes(
                IntervalNumbers.newBuilder().setStart(start).setEnd(end).build(),fps
        );
        while(!fps.isCompleted()){
            //System.out.println("Continue working until receive all prime numbers");
            Thread.sleep(100); // Simulate processing time (1 seg)
        }
    }
    ...
    static void getNevenNumbersAsynchronousCall() throws InterruptedException { // get N even numbers
        ...
        noBlockStub.getEvenNumbers(
                IntNumber.newBuilder().setIntnumber(N).build(),
                evenStream);
